 ...
 */
public class Cmp implements Cpu.ClockedOpcodeConfiguration
{
    /*
     *  CMP
    ...
    {
        // CMP
        config(registry, "0011_100*", "**_***_***", S( 9, CmpRmR.class, "CMP", "[M] <-  R"));
        config(registry, "0011_101*", "**_***_***", S( 9, CmpRmR.class, "CMP", " R  <- [M]"));
        config(registry, "0011_10**", "11_***_***", S( 3, CmpRmR.class, "CMP", " R  <-  R"), true);

        config(registry, "0011_110*",               S( 4, CmpAccImm.class, "CMP", " A   <- I"));
        config(registry, "1000_00**", "**_111_***", S(10, CmpRmImm.class,  "CMP", "[M]  <- I"));
        config(registry, "1000_00**", "11_111_***", S( 4, CmpRmImm.class,  "CMP", " Rm  <- I"), true);
    }
     ...
     *  RegMem with Reg
     */
    public static class CmpRmR extends Cpu.FixedClockOpcode
    {
     ...
     */
    public static class CmpRmImm extends Cpu.FixedClockOpcode {
        @Override
        public void executeClocked(Cpu cpu, int opcode) {
            boolean w = (opcode & 0b0000_0001) == 1;
     ...
     * Imm wth Accumulator
     */
    public static class CmpAccImm extends Cpu.FixedClockOpcode {
        @Override
        public void executeClocked(Cpu cpu, int opcode) {
            boolean w = (opcode & 0b0000_0001) == 1;
